CREATE OR REPLACE VIEW vw_040_cons_result_vota_regiao
AS
SELECT ca.nr_votavel numero,
       ...
       CASE WHEN ca.nm_coligacao IS NOT NULL THEN 'C'
            ...
            ELSE 'P'
       END tp_agremiacao
FROM vw_ft_votos_votavel_regiao vv
...
ON dm.cd_pleito = vv.cd_pleito
   ...
   AND dm.sq_dm_regiao = vv.sq_dm_regiao
INNER JOIN vw_ft_votos_tot_regiao vt
ON vt.cd_pleito = dm.cd_pleito
   AND vt.cd_eleicao = dm.cd_eleicao
   AND vt.cd_cargo_pergunta = ca.cd_cargo_pergunta
   AND vt.sq_dm_regiao = dm.sq_dm_regiao
INNER JOIN vw_ft_secoes_regiao se
