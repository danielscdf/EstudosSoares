CREATE OR REPLACE VIEW vw_056_espelho_bu_candidato_excl
AS
      SELECT sp.cd_pleito,
             s.cd_eleicao,
             sp.sq_secao_pleito_exc,
             el.nm_eleicao,
             s.sg_ue_uf,
             va.sg_ue,
             s.sg_ue_mun,
             ...
             SUM(CASE WHEN vt.cd_tipo_votavel = 4 THEN se.qt_votos ELSE 0 END)
                 ...
                 qt_votos_legenda
        FROM secao_pleito_exc sp                                                                                                                                                                                                                                                              --voto_secao_exc se
             JOIN secao_eleicao s
                 ON sp.cd_pleito = s.cd_pleito
                AND sp.sg_ue_uf = s.sg_ue_uf
                AND sp.sg_ue_mun = s.sg_ue_mun
                AND sp.nr_zona = s.nr_zona
                AND sp.nr_secao = s.nr_secao
             JOIN eleicao el
             ...
             JOIN voto_secao_exc se
                ...
                AND se.sq_secao_pleito_exc = sp.sq_secao_pleito_exc
             LEFT JOIN partido p
