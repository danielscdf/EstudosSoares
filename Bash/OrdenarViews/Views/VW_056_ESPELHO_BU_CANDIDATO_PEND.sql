CREATE OR REPLACE VIEW vw_056_espelho_bu_candidato_pend
AS
      SELECT psp.sq_pendencia_secao_pleito,
             psp.cd_pleito,
             va.cd_eleicao,
             el.nm_eleicao,
             psp.sg_ue_uf,
        ...
        FROM pendencia_secao_pleito psp
             JOIN secao_eleicao s
                 ON psp.cd_pleito = s.cd_pleito
                AND psp.sg_ue_uf = s.sg_ue_uf
                AND psp.sg_ue_mun = s.sg_ue_mun
                AND psp.nr_zona = s.nr_zona
                AND psp.nr_secao = s.nr_secao
             JOIN eleicao el
             ...
             JOIN pendencia_voto_secao se
                 ON s.cd_pleito = se.cd_pleito
                AND vt.cd_pleito = se.cd_pleito
                AND s.cd_eleicao = se.cd_eleicao
                AND vt.cd_eleicao = se.cd_eleicao
                ...
                AND se.sq_pendencia_secao_pleito = psp.sq_pendencia_secao_pleito
             LEFT JOIN partido p
