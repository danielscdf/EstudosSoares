CREATE OR REPLACE VIEW vw_040_cons_result_vota_uf_ar1
AS
SELECT ca.nr_votavel numero,
       ca.nm_cargo_pergunta nm_pergunta,
       ca.nm_votavel nm_cand_resposta_urna,
       ...
       ca.nr_partido,
       COALESCE(ca.nm_coligacao, ca.nm_federacao, ca.nr_partido ||' - ' ||ca.nm_partido) nm_partido_coligacao_completo,
...
WHERE ca.cd_tipo_cargo_pergunta = 3
  AND ca.cd_tipo_votavel_destinacao <> 8
