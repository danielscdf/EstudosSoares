...
adicionar_produto() {
  ...
  #VALUES (COALESCE((SELECT id FROM produtos WHERE nome="$nome"), (SELECT max(id) FROM produtos WHERE nome="$nome"), 0) + 1, "$nome", COALESCE((SELECT quantidade FROM produtos WHERE nome="$nome"), 0) + $quantidade, $preco);

  sqlite3 estoque.db <<EOF
  INSERT OR REPLACE INTO produtos (id, nome, quantidade, preco)
  VALUES (COALESCE((SELECT id FROM produtos WHERE nome='$name'), (SELECT max(id) FROM produtos), 0) + 1, '$nome', COALESCE((SELECT quantidade FROM produtos WHERE nome='$name'), 0) + $quantidade, $preco);
EOF
